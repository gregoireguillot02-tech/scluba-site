...
const safeUrlSchema = z
  ...
  .refine(
    ...
    { message: 'URL refusée (interne / IP / scheme non autorisé)' },
  );
...
const payloadSchema = z.object({
  ...
  source_logo_url: safeUrlSchema.nullable().optional(),
  source_photo_url: safeUrlSchema.nullable().optional(),
...
async function persistAsset(args: {
  ...
  // Defense-in-depth: re-validate the URL through the SSRF guard before
  // fetching. The browser round-trip cannot be trusted to preserve the
  // preview's URL — an attacker who controls the second POST could swap in
  // an internal IP that the zod refine missed.
  try {
    assertSafeUrl(args.sourceUrl);
  } catch (err) {
    console.error(`[api/ops/clubs/from-import] ${args.kind} url rejected`, err);
    return null;
  }
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    console.error('[api/ops/clubs/from-import] unhandled', err);
    if (err instanceof SafeFetchError) {
      return new Response(JSON.stringify({ ok: false, error: 'URL refusée' }), {
        status: 400, headers: NO_STORE_HEADERS,
      });
    }
    return new Response(JSON.stringify({ ok: false, error: 'Create failed — voir les logs ops' }), {
      status: 500, headers: NO_STORE_HEADERS,
    });
  }
};
